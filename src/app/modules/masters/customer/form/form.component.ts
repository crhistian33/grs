...
import { CUSTOMER_FORM_FIELDS } from '@form-fields/customer-fields';
...
export class FormComponent implements OnDestroy {
  ...
  formFields: IFormField[] = CUSTOMER_FORM_FIELDS;

  submitFormData(formData: RequestFormData) {
    const { id, data, close, onReset } = formData;

    const action = id
    ? new CustomerActions.Update(id, data)
    : new CustomerActions.Create(data);
