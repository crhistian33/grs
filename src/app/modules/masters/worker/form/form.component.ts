...
import { WORKER_FORM_FIELDS } from '@form-fields/worker-fields';
...
export class FormComponent implements OnDestroy {
  ...
  formFields: IFormField[] = WORKER_FORM_FIELDS;

  submitFormData(formData: RequestFormData) {
    const { id, data, close, onReset } = formData;

    const action = id
    ? new WorkerActions.Update(id, data)
    : new WorkerActions.Create(data);
